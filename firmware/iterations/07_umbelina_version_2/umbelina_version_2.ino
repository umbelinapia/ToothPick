...
const int CAM_J1 = 4200;
const int CAM_J2 = 8000;
const int CAM_J3 = 0;

const int box_x = 400;
const int box_y = J2_MAX;
...
void cylinderPick(long cylJ1, long cylJ2) {
  Serial.println("Starting cylinder pick sequence...");

  if (cylJ1 < J1.minPos || cylJ1 > J1.maxPos) {
    Serial.println("ERROR: cylinder J1 target out of range.");
    return;
  }

  if (cylJ2 < J2.minPos || cylJ2 > J2.maxPos) {
    Serial.println("ERROR: cylinder J2 target out of range.");
    return;
  }
  ...
  Serial.println("Cylinder pick sequence complete.");
}
...
void executeCommand(String cmd) {
  ...
  if (cmd.startsWith("cylinder(") && cmd.endsWith(")")) {
    ...
    if (comma == -1) {
      Serial.println("Invalid cylinder format. Use: cylinder(x,y)");
      return;
    }

    String xStr = inside.substring(0, comma);
    String yStr = inside.substring(comma + 1);

    long cylJ1 = xStr.toInt();
    long cylJ2 = yStr.toInt();

    cylinderPick(cylJ1, cylJ2);
    return;
  }
  ...
  // flip()
  if (cmd.startsWith("flip(") && cmd.endsWith(")")) {
    int start = cmd.indexOf('(');
    int end = cmd.lastIndexOf(')');
    String inside = cmd.substring(start + 1, end);

    int angle = inside.toInt();
