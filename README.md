...
worker by hand, with a 1% misorientation rate, and it's no longer safe as
the line moves to remote, unmanned automation. Our aim was to design, build,
and test an automated system that locates, flips, and places the disc
reliably, with no person in the loop.
...
Cartesian/joint coordinate, so no inverse-kinematics solve is needed to
convert a target position into motor commands, which kept both the
mechanical build and the control code simpler. Three stepper motors drive
the three linear joints (J1, J2, J3), and two micro servos handle the
end-effector: one opens and closes the gripper, the other flips it between
"pick" and "drop" orientation.
...
`cameraPose()`. Every command blocks until the arm reports back `DONE` or
`ERROR: ...`, so the caller always knows exactly when a move has actually
finished. Two safety interlocks are enforced in the firmware itself, not
just in the higher-level script: the gripper can only flip when J3 is at its
true home position, and it can only drop at the destination when J1 is at
home, the only two positions where those motions are physically safe.
...
close to the front edge of the workspace, a separate "edge routine" kicks in:
the gripper approaches from the side at that boundary instead of straight
down, since a normal vertical approach would risk pushing the disc off the
edge.
...
frames). The host's loop captures an image, processes it, and once it has a
valid detection, walks through the pick sequence one command at a time:
